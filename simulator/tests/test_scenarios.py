...
def test_breach_sequence_is_deterministic() -> None:
    scenario = load_scenario(scenario_path("breach-door-open"))
    assert temperatures("breach-door-open") == [4.4, 6.0, 8.4, 9.2, 10.1]
    assert [step.door_state for step in scenario.steps] == ["CLOSED", "OPEN", "OPEN", "OPEN", "OPEN"]
...
def test_recovery_returns_below_threshold_and_stays_there() -> None:
    ...
    assert len(values[first_recovered:]) >= 7
